...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    elif cfg.DATALOADER.SAMPLER == 'softmax_at':
        def loss_func(score, feat, target):
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                return xent(score, target) + cfg.LOSS.WEIGHT_AT_LOSS * at_loss(feat, target)  # add expAT Loss by yhr.
            else:
                return F.cross_entropy(score, target) + cfg.LOSS.WEIGHT_AT_LOSS * at_loss(feat, target)  # add expAT Loss by yhr.
...
def make_loss_with_center(cfg, num_classes):    # modified by gu
    ...
    elif cfg.MODEL.METRIC_LOSS_TYPE == 'expAT_center':
        at_loss = expATLoss()
        center_criterion = CenterLoss(num_classes=num_classes, feat_dim=feat_dim, use_gpu=True)  # center loss
    ...
    def loss_func(score, feat, target):
        if cfg.MODEL.METRIC_LOSS_TYPE == 'center':
            ...
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                return xent(score, target) + \
                        cfg.LOSS.WEIGHT_AT_LOSS * at_loss(feat, target) + \
                        cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat, target)  
            else:
                return F.cross_entropy(score, target) + \
                        cfg.LOSS.WEIGHT_AT_LOSS * at_loss(feat, target) + \
            ...
            print('expected METRIC_LOSS_TYPE with center should be expAT_center, center,'
                  ' range_center, triplet_center, triplet_range_center '
                  'but got {}'.format(cfg.MODEL.METRIC_LOSS_TYPE))
